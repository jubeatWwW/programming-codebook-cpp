...
namespace sorted_and_rotated_array_search {
    ...
    LN single_improved(T arr[], LN n, T target) {
        return improved_search(arr, 0, n - 1, target);
    }
    ...
    */
    ...
    LN min(T arr[], LN n) {
        LN pivot = find_pivot(arr, 0, n - 1);
        return arr[(pivot + n - 1) % n];
    }

    template<typename T, typename LN = int>
    LN max_hamming_distance(T arr[], LN n) {
        T* double_len_arr = new T[n * 2 + 1];
        for (LN i = 0; i < n; i += 1) {
            double_len_arr[i] = arr[i];
            double_len_arr[i + n] = arr[i];
        }

        LN max_hamming = numeric_limits<LN>::min();

        for (LN start_pos = 1; start_pos < n; start_pos += 1) {
            LN hamming = 0;
            for (LN comp_pos = 0; comp_pos < n; comp_pos += 1) {
                if (arr[comp_pos] != double_len_arr[start_pos + comp_pos]) {
                    hamming += 1;
                }
            }
                            // n is the max hamming distance anyway
            if (hamming == n) {
                return n;
            }

            max_hamming = max(hamming, max_hamming);
        }
